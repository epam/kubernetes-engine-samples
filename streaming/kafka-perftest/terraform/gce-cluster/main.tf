...
# resource "google_compute_disk" "kafka-disks-opt" {
#   count                  = 3
#   name                   = "gce-kafka-disk-opt-${count.index}"
#   type                   = "hyperdisk-balanced"  # Hyperdisk Balanced
#   provisioned_iops       = "6000"
#   provisioned_throughput = "700"
#   size                   = 300                    # Size in GB
...
# resource "random_uuid" "kafka_cluster_id_opt" {}
...
# resource "google_compute_instance" "kafka_opt" {
#   count        = 3
#   name         = "gce-kafka-broker-opt-${count.index}"
#   machine_type = var.machine_type
#   # zone         = var.zones[count.index] # Assign each broker to a different zone
#   zone         = "us-central1-a"
...
#   # Attach persistent Hyperdisk for Kafka logs
#   attached_disk {
#     source      = google_compute_disk.kafka-disks-opt[count.index].id
#     device_name = "kafka-data-disk-opt-${count.index}"
#   }
...
#   metadata = {
#     enable-oslogin : "TRUE"
#     kafka-cluster-id = random_uuid.kafka_cluster_id_opt.result
#   }  
...
#   if ! mount | grep -q "$${KAFKA_ROOT}"; then
#     sudo mkdir -p $${KAFKA_ROOT}
#     # sudo mkfs.ext4 -m 0 -F /dev/disk/by-id/google-kafka-data-disk-opt-${count.index} || true
#     sudo mkfs.xfs -f /dev/disk/by-id/google-kafka-data-disk-opt-${count.index} || true
#     sudo mount -o defaults /dev/disk/by-id/google-kafka-data-disk-opt-${count.index} $${KAFKA_ROOT}
#     # echo "/dev/disk/by-id/google-kafka-data-disk-opt-${count.index} $${KAFKA_ROOT} ext4 defaults,nofail 0 2" | sudo tee -a /etc/fstab
#     echo "/dev/disk/by-id/google-kafka-data-disk-opt-${count.index} $${KAFKA_ROOT} xfs defaults,nofail 0 2" | sudo tee -a /etc/fstab
#   else
#     echo "$(date) Disk is already mounted on $${KAFKA_ROOT}. Skipping disk formatting."
#   fi
...
#   BROKER_IP=$(hostname -I | awk '{print $1}')
#   BROKER_ID=${count.index}
#   BROKER_HOSTNAME="gce-kafka-broker-opt-$${BROKER_ID}"
#   cat <<EOF | sudo tee /opt/kafka/config/server.properties
# broker.id=$${BROKER_ID}
# log.dirs=$${LOG_DIR}
# listeners=PLAINTEXT://$${BROKER_IP}:9092,CONTROLLER://$${BROKER_IP}:9093
# advertised.listeners=PLAINTEXT://$${BROKER_HOSTNAME}:9092
# node.id=$${BROKER_ID}
# controller.quorum.voters=0@gce-kafka-broker-opt-0:9093,1@gce-kafka-broker-opt-1:9093,2@gce-kafka-broker-opt-2:9093
# controller.listener.names=CONTROLLER
# process.roles=broker,controller
# num.network.threads=3
# num.io.threads=8
